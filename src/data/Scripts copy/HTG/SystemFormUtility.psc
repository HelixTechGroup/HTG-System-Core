...
Form Function CreateForm(Int aiFormId, String modName = "HTG-System-Core") Global
    ...
    If kForm != None
        LogObjectGlobal(kForm, "HTG:SystemFormUtility.CreateForm(" + aiFormId + ")")
        return kForm
    EndIf

    LogObjectGlobal(kForm, "HTG:SystemFormUtility.CreateForm: Unable to create Form: " + aiFormId)
    return None
EndFunction
...
ObjectReference Function CreateReference(ObjectReference akSpawnPoint, Form akForm, Bool abPersist = False, Alias akAlias = None) Global
    If akForm && akSpawnPoint
        ...
        If ref
            ...
            LogObjectGlobal(ref, "HTG:SystemFormUtility.CreateReference(" + akForm + ")")
            return ref
        EndIf
    EndIf

    LogErrorGlobal(akSpawnPoint, "HTG:SystemFormUtility.CreateReference: Unable to create Form: " + akForm)
    return None
EndFunction
...
ObjectReference Function CreateReferenceFromExisting(Actor akActor, Form akForm, Alias akAlias = None) Global
    If akForm && akActor
        ...
        If ref != None 
            ...
            LogObjectGlobal(ref, "HTG:SystemFormUtility.CreateReferenceFromExisting(" + akForm + ")")
            return ref
        EndIf        
    EndIf
