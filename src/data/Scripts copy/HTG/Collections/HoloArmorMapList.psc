...
Int Function _FindStruct(String asVarName, Var akElement)
    HoloArmorMap[] kArray = new HoloArmorMap[0]
    Int i
    Int res = -1
    While i < Count
        HoloArmorMap kMember = GetAt(i)
        kArray.Add(kMember)
        i += 1
    EndWhile

    If asVarName == "ArmorPiece"
        res = kArray.FindStruct("ArmorPiece", akElement as Armor)
    ElseIf asVarName == "ArmorMod"
        res = kArray.FindStruct("ArmorMod", akElement as ObjectMod)
    EndIf
