Scriptname HTG:SystemArmorUtility extends ScriptObject Hidden
import HTG:Structs
...
Keyword Property StarbornManufacturer Mandatory Const Auto
Keyword Property BackpackMod Mandatory Const Auto
Keyword Property HelmetMod Mandatory Const Auto
...
Keyword[] Function GetAllArmorTypes(Form akForm)
    ; TryLockGuard _armorCheckGuard
    Keyword[] kRes = new Keyword[0]
    Int i = 0
    Int iCount = ArmorTypes.GetSize()
    Bool bFoundOnce

    While i < iCount
        Keyword kw = ArmorTypes.GetAt(i) as Keyword
        If (akForm.HasKeyword(kw))
            kRes.Add(kw)
        EndIf
        i += 1
    EndWhile
...
Bool Function HasMultipleArmorTypes(Form akForm)
    Bool bRes
    Int i = 0
    Int iCount = ArmorTypes.GetSize()
    Bool bFoundOnce
...
Bool Function IsStarbornArmor(Form akForm)
    return akForm.HasKeyword(StarbornManufacturer)
EndFunction
