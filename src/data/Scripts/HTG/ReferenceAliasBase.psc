Scriptname HTG:ReferenceAliasBase extends ReferenceAlias
{Base HTG ReferenceAlias}
import HTG
import HTG:Structs
...
SystemTimerDefaults _timerDefaults
SystemWaitDefaults _waitDefaults
Bool _isInitialized
Bool _isInitialRun
...
Event OnInit()
    ...
    _waitDefaults = new SystemWaitDefaults
    _timerDefaults = new SystemTimerDefaults
EndEvent
...
Event OnTimer(Int aiTimerID)
    ...
    If aiTimerID == _timerIds.InitializeId
        ...
        TryLockGuard _initializeTimerGuard
            ...
            ;     timerId = Utilities.Timers.SystemIds.InitialRunId
            EndIf
            _initializeTimerStarted = False
        EndTryLockGuard
    ...
    ElseIf aiTimerID == _timerIds.InitialRunId
        ...
        LogObjectGlobal(Self, "ReadyTimer - Completed Initial Run.")
    ...
    ElseIf aiTimerID == _timerIds.MainId
        ...
        Int kMaxStarWait = _waitDefaults.MaxCycles ; Utilities.Timers.WaitDefaults.MaxCycles
        Bool kShouldWait = GetOwningQuest().IsStarting() || !GetOwningQuest().IsRunning()
        While (kShouldWait)
            WaitExt(_waitDefaults.Time)
            If i <= kMaxStarWait
                i += 1
...
Event HTG:ReferenceAliasBase.OnInitialRun(HTG:ReferenceAliasBase akSender, Var[] akArgs)
EndEvent

Event HTG:ReferenceAliasBase.OnMain(HTG:ReferenceAliasBase akSender, Var[] akArgs)
EndEvent
