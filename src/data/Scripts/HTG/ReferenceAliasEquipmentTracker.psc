Scriptname HTG:ReferenceAliasEquipmentTracker extends HTG:ReferenceAliasInventoryTracker
...
Bool _checkStarted
Int _allowSuffocation

Event OnAliasChanged(ObjectReference akObject, bool abRemove)
    Parent.OnAliasChanged(akObject, abRemove)

    StartTimer(Utilities.Timers.Defaults.Interval, _setupTimerId)
EndEvent
    ...
    if aiTimerID == _setupTimerId
        ...
        TryLockGuard _setupTimerGuard, _trackedPlayerArmorGuard
            ...
            ElseIf !_setupComplete && _currentSetupTimerCycle == _maxSetupTimerCycle
                Logger.ErrorEx("ReferenceAliasEquipmentTracker could not be Initialized")
                return
            Else
                Logger.Log("InitializeTimer - Is Initialized. Starting ReadyTimer")
                timerId = Utilities.Timers.SystemIds.MainId
            EndIf
...
Bool Function _SetupActor()
    ...
    ; Else
        ...
        _setupComplete = _RegisterEquipment()
...
Bool Function _RegisterEquipment()
    Int fI = 0
    Bool bHatFound
    Bool bClothesFound
    Bool bHelmetFound
    ...
    If !_isPlayerInitialized
        WaitForCombatEnd()
        _allowSuffocation = AllowSuffocation.GetValueInt()
        AllowSuffocation.SetValueInt(0)
                ...
                If !kActor.IsEquipped(kArmor)
                    ...
                    Else
                        TryLockGuard _trackedPlayerArmorGuard
                            CurrentEquipment.Remove(kArmor)
                        EndTryLockGuard
                    EndIf
                    Wait(1.5)
                Else
