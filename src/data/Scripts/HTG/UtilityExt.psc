...
Bool Function AddLeveledItemToActor(Actor akActor, LeveledItem akItem, Form akForm  = None, int count = 1, bool clearAllExisting = false, bool autoEquip = false, Alias akAlias = None) Global
    Int c = 0
    Bool equip = False
    
    If !IsNone(akForm)
        Armor ta = akForm as Armor
        ...
        If c > 0 && clearAllExisting
            ...
            akActor.RemoveItem(akForm, c, abSilent = true)
            WaitExt(0.50)
        ElseIf c > 0
            If autoEquip && equip
    ...
    If !IsNone(akForm)
        WaitExt(0.666)
        int nc = akActor.GetItemCount(akForm)
        If nc > 0
            If autoEquip && equip
                akActor.EquipItem(akForm, abSilent = true)
            EndIf
        EndIf
        ;WaitExt(0.25)
